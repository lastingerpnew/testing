...
class Chat extends ERP_Controller{
    ...
    public function __construct()
    {
		...
		/*$this->settings['css'][] = 'assets/css/camera.css';
		$this->settings['css'][] = 'assets/css/bootstrap.css';
		$this->settings['css'][] = 'assets/css/bootstrap-theme.css';
		$this->settings['css'][] = 'assets/css/ie10-viewport-bug-workaround.css';*/
		$this->settings['css'][] = 'assets/modules/chat/css/theme.css';
		$this->settings['css'][] = 'assets/modules/chat/css/chat.css';
		$this->scripts['js'][] = 'assets/modules/chat/js/chat.js';
		$this->scripts['js'][] = 'assets/modules/chat/js/ie-emulation-modes-warning.js';
		$this->scripts['js'][] = 'assets/modules/chat/js/docs.min.js';
		$this->scripts['js'][] = 'assets/modules/chat/js/ie10-viewport-bug-workaround.js'; 
		$this->scripts['js'][] = 'assets/modules/chat/js/dashboard.js'; 
    ...
    public function index() {
		...
		$this->breadcrumb->add('Chat', base_url() . 'chat');
		$this->settings['breadcrumbs'] = $this->breadcrumb->output();
		$this->settings['pageTitle'] = 'Chatting';

			$data1['record'] = $this->user->get($this->session->userdata('user_id'));
			
			
			//$this->load->view('dashboard/index', $data1);
			//$this->_render_template('dashboard/index', $data1);
			
			
            // $this->load->view('template/main_template', 'dashboard/index', $data1);

        if (isset($_POST['submit'])) {
           $id = $this->uri->segment(3);
			
            $config['upload_path']          = './assets/modules/chat/uploads/';
            $config['allowed_types']        = 'gif|jpg|png|pdf|docx|doc|sql|xlsx|xls|ppt|pptx|php';
			//pre($config);die('there');
            ...
            if (!$this->upload->do_upload('userfile')) {
                ...
                redirect('dashboard/index/'.$chat_id);
            }
    ...
    public function redirect()
    {
        ...
        if ($result == 1) {
			//echo $this->session->userdata('chat_id');die('there');
...
//pre($this->view_data);
       ...
       //$this->_render_template('template/main_template', 'chat/index', $this->view_data);
	  // echo $this->index($this->view_data);
    }
    ...
    public function _get_chats_messages($chat_id)
    {
		...
		//pre($chats_messages.'23222');die('dfdfdfdfdfdfdfdfdfdfd'); 
        ...
        if ($chats_messages->num_rows() > 0) {
            ...
            foreach ($chats_messages->result() as $chats_messages) {
				//
                $record = $this->db->get_where('user_detail', ['u_id' => $chats_messages->user_id])->row_array();
				//pre($record);die();
                ...
                if ($chats_messages->is_image == '0') {
                    if ($chats_messages->is_doc == '1') {
                        $chats_messages_html .=
                        '<p><li ' . $li_class . '>'
                            . '<p class="message_content"><img src="'.base_url().'assets/modules/users/uploads/'.$avatar.'" height="25" width="25">
                    ...
                    $chats_messages_html .=
                        '<p><li ' . $li_class . '>'
                            . '<p class="message_content"><img src="'.base_url().'assets/modules/users/uploads/'.$avatar.'" height="25" width="25">
                                <a href="'.base_url().'uploads/'.$chats_messages->content.'" target="_BLANK">
                                    <img src="'.base_url().'assets/modules/users/uploads/'.$chats_messages->content.'" width="100" height="100" />
                                </a>
                            </p>
                            <span class="chat_message_header"><b>' .
                                $chats_messages->timestamp .
