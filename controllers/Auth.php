    ...
    public function login()
    {
        if (isset($_POST['submit']))
        {
            ...
            if ($verify == 1) {
                ...
                $this->session->set_userdata('error', 'Please ask the admin to provide verification of your account!');
                redirect('auth/login');
            } else {
                /* Destory session if failed */
                // $this->session->sess_destroy();
                $this->session->set_userdata(['error' => 'Error!! Invalid username and password !!']);
                redirect('auth/login');
            }
